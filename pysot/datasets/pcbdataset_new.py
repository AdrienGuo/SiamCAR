...
class PCBDataset(Dataset):
    def __init__(self, args, type: str, loader=default_loader):
        """ 代號
            z: template
            x: search
        Args:
    ...
    def __getitem__(self, idx):
        ...
        # 再從上面裁切出 template image (z_img)。
        # z_box: (1, [x1, y1, x2, y2])
        # gt_boxes: (num, [x1, y1, x2, y2])
        x_img, gt_boxes, z_box, r, spatium = self.pcb_crop.get_search(x_img, gt_boxes.copy(), z_box.copy())
        z_img = self.pcb_crop.get_template(x_img, z_box.copy(), self.args.bg)
