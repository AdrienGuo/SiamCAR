...
from pysot.datasets.pcb_crop_tri_old import PCBCrop
...
class PCBDataset():
    ...
    def __init__(self, args, mode) -> None:
        ...
        self.z_size = cfg.TRACK.EXEMPLAR_SIZE
        self.x_size = cfg.TRACK.INSTANCE_SIZE

        self.mode = mode
        self.pcb_crop = PCBCrop(
            template_size=self.z_size,
            search_size=self.x_size
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "test":
            channel_average = np.mean(x_img, axis=(0, 1))  # 用這張影像的平均值當作 padding
            channel_average = np.floor(channel_average)
        z_img, z_box, r = self.pcb_crop.get_template(
            z_img, z_box, padding=channel_average)  # array 真的要小心處理，因為他們的 address 都是一樣的
        x_img = self.pcb_crop.get_search(
            x_img, gt_boxes, r, padding=channel_average)

        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ########################
        # 創 directory
        ########################
        # dir = f"./image_check/x{self.x_size}"
        # img_name = img_path.split('/')[-1]
