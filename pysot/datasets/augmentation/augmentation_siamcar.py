...
from pysot.datasets.box_transforms import cxcywhtox1y1x2y2, x1y1x2y2tocxcywh
...
class Augmentation:
    ...
    def _shift_scale_aug(self, image, bbox, crop_bbox, size):
        ...
        crop_bbox_center = x1y1x2y2tocxcywh(crop_bbox)
        if self.scale:
            scale_x = (1.0 + Augmentation.random() * self.scale)
            scale_y = (1.0 + Augmentation.random() * self.scale)
            h, w = crop_bbox_center[3], crop_bbox_center[2]
            ...
            crop_bbox_center = np.array([
                crop_bbox_center[0],
                crop_bbox_center[1],
                crop_bbox_center[2] * scale_x,
                crop_bbox_center[3] * scale_y
            ])

        # crop_bbox = center2corner(crop_bbox_center)
        crop_bbox = cxcywhtox1y1x2y2(crop_bbox_center)
    ...
    def _flip_aug(self, image, bbox):
        ...
        bbox = np.array([
            width - 1 - bbox[2], bbox[1],
            width - 1 - bbox[0], bbox[3]
        ])
        return image, bbox
    ...
    def __call__(self, image, bbox, size, gray=False):
        ...
        if self.color > np.random.random():
            # if self.color > rng.random():
            image = self._color_aug(image)

        # blur augmentation
        # blur_p = np.random.random()
        # blur_p = rng.random()
        # print(blur_p)
        ...
        if self.flip and self.flip > np.random.random():
            # if self.flip and self.flip > rng.random():
